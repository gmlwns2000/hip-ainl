...
class HiPAttentionArgs:
    ...
    multi_branch_layer_all : int = None,
    multi_branch_per_layer : int = None,
    multi_branch_true_iter : int = None,
    multi_branch_ret_ratio: float = None,
    multi_branch_ret_ratio_select_all: bool = None,
    multi_branch_true_iter_cnt: int = None,
    multi_branch_true_iter_str: str = None,
    
    layer_id: int =None,

    k_ret_ratio : float = None,
...
def masking_iteration_draft_cuda_initialize(
    ...
    # out
    # B, cdiv_python(TDST, block_size_q), G * mask_block_k * multi_branch_ratio_per_layer
    ...
    if TSRC <= mask_k:
        # tl.device_print('###### BUG ######')
        idx_bk = tl.arange(0, BLOCK_MASK_BLOCK_K * multi_branch_ratio_per_layer) # TODO MUST CHECK
        mask_bk = idx_bk < BSRC
    ...
    else: # INDICES : B, cdiv_python(TDST, block_size_q), G * mask_block_k * multi_branch_ratio_per_layer
        idx_bk_origin = tl.arange(0, BLOCK_MASK_BLOCK_K) # triton.next_power_of_2(mask_block_k)
        mask_bk_origin = idx_bk_origin < mask_block_k * G
        ...
        indices = tl.minimum(
            ((MAX_BSRC * idx_group + (BSRC / mask_block_k * idx_bk_origin)).to(tl.int32) // ALIGN_STEP) * ALIGN_STEP, 
            (MAX_BSRC * idx_group + BSRC).to(tl.int32)
        )
        next_indices = tl.minimum(
            ((MAX_BSRC * idx_group + (BSRC / mask_block_k * (idx_bk_origin + 1))).to(tl.int32) // ALIGN_STEP) * ALIGN_STEP, 
            (MAX_BSRC * idx_group + BSRC).to(tl.int32)
        )
        group_sizes = tl.maximum(0, tl.minimum(BSRC, next_indices - indices)).to(tl.int32)
        if INDICES_SEED is not None:
            if ks == (mask_block_k * G):
                indices = tl.load( # [G * mask_block_k]
                    INDICES_SEED +\
                        idx_b * stride_indices_seed_b +\
                        idx_bdst * stride_indices_seed_bdst +\
                        (idx_group * mask_block_k + idx_bk_origin) * stride_indices_seed_bk,
                    mask=mask_bk_origin,
                ...
                indices_next = tl.load(
                    INDICES_SEED +\
                        idx_b * stride_indices_seed_b +\
                        idx_bdst * stride_indices_seed_bdst +\
                        (idx_group * mask_block_k + idx_bk_origin + 1) * stride_indices_seed_bk,
                    mask=(
                        mask_bk_origin &
                        ((idx_group * mask_block_k + idx_bk_origin + 1) < (BLOCK_MASK_BLOCK_K * G))
                    ),
                    other=G * MAX_BSRC,
                ).to(tl.int32)
        ...
        # NOTE T_GROUP_SIZE must be placed here, before broadcast
        ...
        group_sizes = tl.broadcast_to(group_sizes[None, :], (multi_branch_ratio_per_layer, BLOCK_MASK_BLOCK_K))
        group_sizes = tl.ravel(tl.trans(group_sizes))

        idx_bk = tl.arange(0, BLOCK_MASK_BLOCK_K * multi_branch_ratio_per_layer) # triton.next_power_of_2(mask_block_k)
        mask_bk = idx_bk < mask_block_k * G * multi_branch_ratio_per_layer
...
def masking_iteration_draft_cuda_dup_and_score_calc_score(
    idx_key_blocks, # 2 * multi_branch_ration - 1, 1, 2
    ...
    if KEY_ACCESS_LOG is not None:
        ...
        mask_access = tl.ravel(tl.broadcast_to( # [BLOCK_BK, BLOCK_SIZE_K // BLOCK_STRIDE_K]
            mask_key_blocks[:, None], # [BLOCK_BK, KEY_DUP] ~ (dupped_group_size > 0) & mask_bk_dup
            BLOCK_BK * KEY_DUP, BLOCK_SIZE_K // BLOCK_STRIDE_K
        ))
    ...
    for i_group in tl.range(0, G):
        ...
        if G == 1: # G = topk_head_group_size = 1
            mask_keys = tl.broadcast_to(
                mask_key_blocks[:, None], # BLOCK_BK * KEY_DUP
                BLOCK_BK * KEY_DUP, 
                BLOCK_SIZE_K // BLOCK_STRIDE_K
            )
            mask_keys = tl.ravel(mask_keys)[None, :] # (BLOCK_BK * KEY_DUP)* (BLOCK_SIZE_K // BLOCK_STRIDE_K)[None, :]
...
@triton.jit
def multi_branch_masking_iteration_draft_cuda_dup_and_score_calc_score(
    KEY_DUP: tl.constexpr,
    ...
    idx_dupped_indices_bk = tl.arange(0, BLOCK_BK)[:, None] * (2 * multi_branch_ratio_per_iter) + tl.arange(1, 2 * multi_branch_ratio_per_iter)[None, :]
    idx_dupped_indices_bk = tl.ravel(idx_dupped_indices_bk)
    mask_bk_dup = idx_dupped_indices_bk < (BK * 2 * multi_branch_ratio_per_iter * G)
    
    idx_key_blocks = tl.load( # [(2 * multi_branch_ratio - 1) * BLOCK_BK] flatten
        DUPPED_INDICES +\
    ...
    mask_key_blocks = (dupped_group_size > 0) & mask_bk_dup # [(2 * multi_branch_ratio - 1) * BLOCK_BK]
...
def multi_branch_dupped_group_sizes_and_sampling_method_cal(
    ...
    branch_location = grid_i
    # idx_bk_dup : idx_bk_dup = pid_bbk * BLOCK_BK * 2 * multi_branch_ratio + tl.arange(0, BLOCK_BK * 2 * multi_branch_ratio)
    
    if mask is None:
        mask = tl.full((BLOCK_BK,), 1, dtype=tl.int8) # [BLOCK_BK]
        mask = mask.to(tl.int1)
    else:
        mask = ~mask.reshape(BLOCK_BK) # [BLOCK_BK] # mask = tl.expand_dims(group_sizes, axis=1) <= M2; False have to compute as usual
    
    # load only those indices
    idx_bk_dup_bl = tl.arange(0, BLOCK_BK) * (2 * multi_branch_ratio * G) + branch_location
    idx_bk_dup_bl_1 = tl.arange(0, BLOCK_BK) * (2 * multi_branch_ratio * G) + branch_location + 1
    
    dupped_indices_bl = tl.load( # BLOCK_BK or less
        DUPPED_INDICES +\
            idx_b * stride_dupped_indices_b +\
            idx_bdst * stride_dupped_indices_bdst +\
            idx_bk_dup_bl * stride_dupped_indices_bk,
        mask = (idx_bk_dup_bl < (BK * 2 * G * multi_branch_ratio)) & mask,
        cache_modifier=DEFAULT_CACHE_MODIFIER,
    )
    
    # dupped_group_sizes
    if branch_location != 2 * multi_branch_ratio -1:
        # in (branch_location) idx, dupped_group_sizes stores (branch_location + 1)_dupped_indices - (branch_location)_dupped_indices
        dupped_indices_bl_1 = tl.load( # BLOCK_BK or less
            DUPPED_INDICES +\
                idx_b * stride_dupped_indices_b +\
                idx_bdst * stride_dupped_indices_bdst +\
                idx_bk_dup_bl_1 * stride_dupped_indices_bk,
            mask = (idx_bk_dup_bl_1 < (BK * 2 * G * multi_branch_ratio)) & mask,
            cache_modifier=DEFAULT_CACHE_MODIFIER,
        )
        
        dupped_group_sizes_temp = dupped_indices_bl_1 - dupped_indices_bl # BLOCK_BK or less
        ...
        dupped_group_sizes_temp = tl.full((dupped_indices_bl.shape[0], ), 2 * multi_branch_ratio, dupped_indices_bl.dtype) - dupped_indices_bl
...
def multi_branch_dupped_indices_cal(
    ...
    # two indices are done together for dupped_group_sizes
    branch_location = grid_i + 1 # 1 ~ 2 * multi_branch_ratio - 1
    
    if mask is None:
        mask = tl.full((BLOCK_BK * 2 * multi_branch_ratio * G,), 1, dtype=tl.int8)
        mask = mask.to(tl.int1)
    else:
        # mask = tl.expand_dims(group_sizes, axis=1) <= M2 [BLOCK_BK, 1]
        mask = tl.broadcast_to(mask, (BLOCK_BK, 2 * multi_branch_ratio * G)).reshape(BLOCK_BK * 2 * multi_branch_ratio * G)
        mask = ~mask # True if group_sizes is greated than M2
    
    # mask : [BLOCK_BK * 2 * multi_branch_ratio * G]
    # mask_bk_dup : [BLOCK_BK * 2 * multi_branch_ratio]

    dupped_indices = tl.load( # BLOCK_BK * 2 * multi_branch_ratio or less (those chunks that need computation)
        DUPPED_INDICES +\
            idx_b * stride_dupped_indices_b +\
            idx_bdst * stride_dupped_indices_bdst +\
            idx_bk_dup * stride_dupped_indices_bk,
        mask=mask_bk_dup & mask, # NOTE need to load all?
        cache_modifier=DEFAULT_CACHE_MODIFIER,
    )
    dupped_group_sizes = tl.load( # TTTTTTTTFFF & TTTFFFTTTFFF
        DUPPED_GROUP_SIZE +\
            idx_b * stride_dupped_group_size_b +\
            idx_bdst * stride_dupped_group_size_bdst +\
    ...
    tl.device_assert(BRANCH_METHOD != 'half')
    
    idx_bk_mul = tl.arange(0, BLOCK_BK * 2 * multi_branch_ratio)
    branch_condition = (idx_bk_mul % (2 * multi_branch_ratio)) == branch_location
    
    tl.store(
        CONDITION +\
            idx_bk_mul * stride_condition_size_bk_mul,
        value=branch_condition,
        mask=mask_bk_dup & mask,
        cache_modifier=DEFAULT_CACHE_MODIFIER,
    )
    
    branch_condition = tl.load(
        CONDITION +\
            idx_bk_mul * stride_condition_size_bk_mul,
        mask=mask_bk_dup & mask,
        cache_modifier=DEFAULT_CACHE_MODIFIER,
    ...
    if branch_location != 0 and BRANCH_METHOD == 'equal_size':
        dupped_indices = tl.where( # BLOCK_BK * 2 * multi_branch_ratio * G
            branch_condition,
    ...
    elif branch_location != 0 and BRANCH_METHOD == 'random_per_iter':
        dupped_indices = tl.where(
            branch_condition,
            tl.where(
                dupped_group_sizes == 0,
    ...
    tl.store(
        ...
        mask=mask_bk_dup & mask, # TODO check
        cache_modifier=DEFAULT_CACHE_MODIFIER,
    )    
...
def masking_iteration_draft_cuda_dup_and_score_multi_branch_curr(
    ...
    M2:tl.constexpr = tl.constexpr(2 * multi_branch_ratio_per_layer)
    
    if t_group_size <= M2:
        garbage_value = MAX_TSRC + 1
        dupped_indices = tl.full((BLOCK_BK, M2), garbage_value, dtype=indices.dtype)
        group_indices = tl.broadcast_to(tl.expand_dims(tl.arange(0, M2), axis=0), (BLOCK_BK, M2))  # Shape (1, t_group_size)

        group_indices = group_indices + tl.expand_dims(indices, axis=1)
        scatter_indices = tl.broadcast_to(tl.expand_dims(tl.arange(0, M2), axis=0), (BLOCK_BK, M2))

        valid_mask = scatter_indices < tl.expand_dims(group_sizes, axis=1)
        
        dupped_indices = tl.where(valid_mask, group_indices, dupped_indices)
        dupped_group_sizes = tl.full((BLOCK_BK, M2), 1, dtype=group_sizes.dtype)
        # dupped_group_sizes = torch.ones((BLOCK_BK, M2), dtype=group_sizes.dtype)
        
        dupped_indices = tl.reshape(dupped_indices, (BLOCK_BK * M2))
        dupped_group_sizes = tl.reshape(dupped_group_sizes, (BLOCK_BK * M2))
        
        # mask_bk_dup = mask_bk_dup & (dupped_indices < garbage_value) # TODO check whether garbage value is managed well
        ...
        if SCORES_CACHED:
            ...
            # TODO check handling garbage_value
            scores_sampled = masking_iteration_draft_cuda_dup_and_score_calc_score(
                indices_to_sample,
        ...
        mask = tl.expand_dims(group_sizes, axis=1) <= M2

        valid_mask = scatter_indices < tl.expand_dims(group_sizes, axis=1)
        
        dupped_indices = tl.where(valid_mask & mask, group_indices, dupped_indices) # TODO check
        # dupped_indices.scatter_(1, scatter_indices, tl.where(valid_mask & mask, group_indices, dupped_indices))
        dupped_group_sizes = tl.where(mask, tl.full((BLOCK_BK, M2), 1, dtype=group_sizes.dtype),
                                tl.broadcast_to(tl.expand_dims(group_sizes, axis=1), (BLOCK_BK, M2)))
        
        dupped_indices = tl.where(mask, dupped_indices, tl.broadcast_to(tl.expand_dims(indices, axis=1), (BLOCK_BK, M2)))
        
        dupped_indices = tl.reshape(dupped_indices, (BLOCK_BK * M2))
        dupped_group_sizes = tl.reshape(dupped_group_sizes, (BLOCK_BK * M2))
        
        # mask_bk_dup = mask_bk_dup & (dupped_indices < garbage_value) # TODO check : then it should ignore MAX_TSRC 
...
def masking_iteration_draft_cuda_dup_and_score(
    ...
    """
    - mask_k : hip parameters
    - BLOCK_SIZE_K = block_size_k : hip parameters
    - BK = mask_block_k = cdiv_python(mask_k, block_size_k)
    - BLOCK_BK : 
    ...
    """
    not multi_branch_on_layer: multi_branch_ratio_per_iter == 1
...
def masking_iteration_draft_cuda_fused(
    ...
    # #(BBH, GDST, GBH, BSZ)
    # _grid_bbh = GROUP_BH
    _grid_gdst = tl.cdiv(MAX_BDST, GROUP_BDST) # GROUP_BDST = 1 -> MAX_BDST
    ...
    num_groups = tl.minimum(GROUP_BDST, (MAX_BDST - _pid_1 * GROUP_BDST)) # 1
    for i_group in range(num_groups):
        ...
        idx_iteration = 0
        multi_branch_on_once = False
        
        if multi_branch_on_layer:
            tl.static_assert(multi_branch_true_iter >= 0)
        else:
            tl.static_assert(multi_branch_true_iter < 0)
        ...
        while max_group_size > 1:
            ...
            multi_branch_on_once = multi_branch_true_iter>=0 and idx_iteration >= multi_branch_true_iter
            # multi_branch_on_once : True -> multi_branch_ratio_per_layer
            # multi_branch_on_once : False -> 1
            if multi_branch_on_once:
                # multi_branch_ratio_per_iter = multi_branch_ratio_per_layer
                n_program = tl.cdiv(BK * G * multi_branch_ratio_per_layer, BLOCK_BK)
            else:
                n_program = tl.cdiv(BK * G * 1, BLOCK_BK)
            ...
            if multi_branch_on_layer and idx_iteration >= multi_branch_true_iter: # TODO do we need ceiling etc?
                if BRANCH_METHOD == 'random' or BRANCH_METHOD == 'random_per_iter':
                    max_group_size *= 1.4 * (1/(2 * multi_branch_ratio_per_layer))
                elif BRANCH_METHOD == 'half' or BRANCH_METHOD == 'equal_size':
                    max_group_size *= (1/(2 * multi_branch_ratio_per_layer))
                else:
                    raise Exception(BRANCH_METHOD)
            else:
                if BRANCH_METHOD == 'random' or BRANCH_METHOD == 'random_per_iter':
...
def masking_iteration_draft_cuda_initialize_score(
    ...
    pid_bh = tl.program_id(0) % BH # 0 ~ BH - 1
    pid_bk = tl.program_id(0) // BH # 0
    pid_bdst = tl.program_id(1)
    pid_bsz = tl.program_id(2)
    
    idx_bk = pid_bk * BLOCK_BK + tl.arange(0, BLOCK_BK * multi_branch_ratio_per_layer) # BLOCK_BK = mask_k // block_size_k * G
    ...
    if IS_CAUSAL:
        ...
        pos_tdst = tl.full((BLOCK_SIZE_Q // BLOCK_STRIDE_Q, ), value=MAX_TSRC, dtype=tl.int64)
    TSRC = tl.max(pos_tdst)
    TSRC = tl.maximum(0, TSRC - sliding_window_size) # TODO what's sliding_window_size ?
    BSRC = tl.cdiv(TSRC, BLOCK_SIZE_K)
    
    indices = tl.load( # [BLOCK_BK * multi_branch_ratio_per_layer]
        INDICES +\
            idx_b * stride_indices_b +\
            idx_bdst * stride_indices_bdst +\
            idx_bk * stride_indices_bk,
    ...
    scores = masking_iteration_draft_cuda_dup_and_score_calc_score(
        indices,
        KEY_DUP, # multi_branch_ratio_per_layer
        ...
        idx_bdst,
        idx_tdst, mask_tdst, pos_tdst,
        mask_bk, # this goes into dupped_mask
        ...
        multi_branch_ratio_per_layer,
        idx_iteration=-1,
        multi_branch_true_iter=None,
        multi_branch_on_layer=multi_branch_on_layer
    )
...
def masking_iteration_draft( 
    ...
    # TODO apply multi branching in different positions for each layer
    multi_branch_on_layer_first_iter = False
    if (args.multi_branch_layer_list is not None and args.layer_id in args.multi_branch_layer_list) or \
        args.multi_branch_layer_all or (args.layer_id == args.multi_branch_particular_layer) or \
        (args.multi_branch_layer_start is not None and args.multi_branch_layer_till is not None and args.layer_id >= args.multi_branch_layer_start and args.layer_id <= args.multi_branch_layer_till \
        and (args.layer_id - args.multi_branch_layer_start) % args.multi_branch_per_layer == 0):
        if args.multi_branch_true_iter_cnt == 1:
            if args.multi_branch_true_iter != 0 or (args.multi_branch_true_iter_str is not None and args.multi_branch_true_iter_str != 'first'):
                multi_branch_on_layer = True
                assert args.multi_branch_ratio > 1
                multi_branch_ratio_per_layer = args.multi_branch_ratio                
            else: # it's just same as increasing k
                multi_branch_on_layer_first_iter = True
    ...
    if max_group_size is not None:
        ...
        total_iteration = math.ceil(math.log2(max_group_size) + 1)
    else:
        KEY_ACCESS_LEN = args.mask_k * 2 * math.ceil(math.log2(MAX_BSRC) + 1)
        total_iteration = math.ceil(math.log2(MAX_BSRC) + 1)
    
    if multi_branch_on_layer:
        if args.multi_branch_true_iter_str == 'first':
            multi_branch_true_iter = 0
        elif args.multi_branch_true_iter_str == 'first_center':
            multi_branch_true_iter = max(total_iteration // 4, 1)
        elif args.multi_branch_true_iter_str == 'center':
    ...
    assert args.branch_method in ['half', 'equal_size', 'random', 'random_per_iter']
    ...
    BLOCK_MASK_BLOCK_K = triton.next_power_of_2(mask_block_k) # TODO mask_block_k * G ?
    ...
    if (scores_seed is not None) and args.sample_method == 'first':
        ...
        assert B == BSZ * BH
        grid = (
            # BH * triton.cdiv(indices.shape[-1], BLOCK_BK), # G * mask_block_k * multi_branch_ratio_per_layer // BLOCK_BK == multi_branch_ratio_per_layer
            BH * triton.cdiv(indices.shape[-1], BLOCK_BK * multi_branch_ratio_per_layer), # BH
            BDST, 
            BSZ,
        )
        ...
        masking_iteration_draft_cuda_initialize_score[grid](
            ...
            args.sliding_window_size,
            indices.shape[-1],
            BH, mask_block_k, G, TDST, TSRC, HID, KV_HEAD_REPEAT,
    ...
    topk_indices = torch.empty(
        (probs.shape[0], probs.shape[1], G * mask_block_k * multi_branch_ratio_per_layer),
        device=probs.device,
        dtype=torch.int32,
    )
    ...
    if using_fused_iteration:
        ...
        if not scores_cached:
            ...
            BLOCK_BK = 128 // (args.block_size_k // args.block_stride_k) // 2 # TODO should I change this?
        ...
        if multi_branch_on_layer:
            condition = torch.zeros(BLOCK_BK * 2 * multi_branch_ratio_per_layer)
        else:
            condition = None
        ...
        masking_iteration_draft_cuda_fused[grid](
            ...
            *args.args_bq_bsq_bk_bsk(),
            ...
            multi_branch_ratio_per_layer,
            multi_branch_true_iter,
            args.multi_branch_ret_ratio,
            args.multi_branch_true_iter_cnt,
            multi_branch_on_layer,
            condition, *args.safe_stride(condition, 1),
    ...
    if multi_branch_on_layer or multi_branch_on_layer_first_iter:
        ...
        if args.multi_branch_ret_ratio_select_all:
            ...
            assert args.multi_branch_ret_ratio <= args.multi_branch_ratio
            assert args.multi_branch_ratio > 1
            if args.multi_branch_ret_ratio == 1:
                indices = indices[:, :, :mask_block_k * G]
            elif args.multi_branch_ret_ratio < args.multi_branch_ratio: # if same, no change in indices
                indices = indices[:, :, :math.ceil(mask_block_k * args.multi_branch_ret_ratio * G)]
    ...
    if os.getenv('DEBUG_MULTI_BRANCH', '0')== '1':
        ...
        }, f'./cache/llama_multi/metadata_l{args.layer_id}_t{TSRC}.pth')
...
def block_sparse_attention_cuda(
    ...
    if (BK > 0):
        ...
        for i_bk in range(range_start, range_start + (BK * G), BLOCK_BK):
            ...
            mask_bk = (idx_bk < (range_start + BK * G)) & (idx_bk < range_end) # TODO change ??
