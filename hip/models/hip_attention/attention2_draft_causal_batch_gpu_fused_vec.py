...
DEFAULT_CACHE_MODIFIER = tl.constexpr('.cg') # , tl.int32
...
def masking_iteration_draft_cuda_initialize(
    ...
    # TODO NOTE - not considering this bc multi_branching prevents this by assertion

    if TSRC <= mask_k:
        idx_bk = tl.arange(0, BLOCK_MASK_BLOCK_K * multi_branch_ratio_per_layer) # TODO NOTE!!! for bug fix. is this right?
        mask_bk = idx_bk < BSRC
        ...
        indices = (MAX_BSRC * idx_group + (BSRC / mask_block_k * idx_bk_origin)).to(tl.int32)
        
        group_sizes = tl.minimum(
            BSRC, 
            (
                BSRC / mask_block_k * (idx_bk_origin + 1).to(tl.int32) -\
                (BSRC / mask_block_k * idx_bk_origin).to(tl.int32)
            )
        ).to(tl.int32)
...
def multi_branch_dupped_group_sizes_and_sampling_method_cal(
    ...
    idx_bk_dup_bl = tl.arange(0, BLOCK_BK) * (2 * multi_branch_ratio) + branch_location
    idx_bk_dup_bl_1 = tl.arange(0, BLOCK_BK) * (2 * multi_branch_ratio) + branch_location + 1
    
    dupped_indices_bl = tl.load( # BLOCK_BK
        DUPPED_INDICES +\
            idx_b * stride_dupped_indices_b +\
            idx_bdst * stride_dupped_indices_bdst +\
    ...
    if branch_location != 2 * multi_branch_ratio -1:
        ...
        dupped_indices_bl_1 = tl.load( # BLOCK_BK
            DUPPED_INDICES +\
                idx_b * stride_dupped_indices_b +\
                idx_bdst * stride_dupped_indices_bdst +\
...
def multi_branch_dupped_indices_cal(
    ...
    # branch_location = tl.program_id(0) + 1 # 1 ~ 2 * multi_branch_ratio - 1
    branch_location = grid_i + 1
...
def masking_iteration_draft_cuda_dup_and_score(
    ...
    multi_branch_layer_list: tl.constexpr,
    multi_branch_layer_start: tl.constexpr,
    multi_branch_layer_till: tl.constexpr,
    multi_branch_layer_all: tl.constexpr,
    multi_branch_per_layer: tl.constexpr,
    multi_branch_true_iter: tl.constexpr = 0,
    multi_branch_true_iter_str: tl.constexpr =None,
    multi_branch_ret_ratio: tl.constexpr = 1.0,
    multi_branch_true_iter_cnt: tl.constexpr = 1,
    ...
    # tl.static_print(multi_branch_on_layer)
    # tl.static_print(idx_iteration)
    # tl.static_print(multi_branch_true_iter)
    
    if multi_branch_on_layer:
        # tl.device_assert(multi_branch_true_iter is not None)
        # if multi_branch_true_iter is not None:
        if idx_iteration < multi_branch_true_iter:
            # single branching
        ...
        elif idx_iteration == multi_branch_true_iter:
            # multi_branching
            tl.device_assert(multi_branch_ratio_per_iter > 1)
    ...
    if multi_branch_on_layer and idx_iteration == multi_branch_true_iter:
        ...
        # grid = (2 * multi_branch_ratio_per_layer - 1, ) # 0th index should not be changed
        for grid_i in range(2 * multi_branch_ratio_per_layer - 1):
        ...
        # grid = (2 * multi_branch_ratio_per_layer, )
        for grid_i in range(2 * multi_branch_ratio_per_layer):
    ...
    elif multi_branch_on_layer and idx_iteration > multi_branch_true_iter: # SINGLE BRANCH
        # duplicate 2 times -> join
        # idx_iteration < multi_branch_true_iter : multi_branch_ratio == 1
        # idx_iteration > multi_branch_true_iter : multi_branch_ratio > 1
        dupped_indices = tl.reshape(
            tl.join(indices, indices), # indices : BLOCK_BK * multi_branch_ratio_per_layer
            (BLOCK_BK * 2 * multi_branch_ratio_per_layer,),
        )
        dupped_group_sizes = tl.reshape(
    ...
    else: # only single branching layer or idx_iteration < multi_branch_true_iter:
        dupped_indices = tl.reshape(
...
def masking_iteration_draft_cuda_partial_softmax(
    ...
    if multi_branch_on_layer and idx_iteration >= multi_branch_true_iter:
        idx_bk = tl.arange(0, triton.next_power_of_2(2 * G * MASK_BLOCK_K * multi_branch_ratio_per_layer)) # triton.next_power_of_2(scores.shape[-1] == 2 * G * mask_block_k * multi_branch_ratio_per_layer)
        dup_bk = 2 * G * MASK_BLOCK_K * multi_branch_ratio_per_layer
    else:
        idx_bk = tl.arange(0, triton.next_power_of_2(2 * G * MASK_BLOCK_K)) # triton.next_power_of_2(scores.shape[-1] == 2 * G * mask_block_k * multi_branch_ratio_per_layer)
        dup_bk = 2 * G * MASK_BLOCK_K
...
def masking_iteration_draft_cuda_argsort(
    ...
    if multi_branch_on_layer and idx_iteration >= multi_branch_true_iter:
        ...
        idx_bk = tl.arange(0, 2 * BK * G,)
        ids = tl.broadcast_to(tl.arange(0, 2 * BK * G)[None, :], \
                          (BLOCK_BDST, 2 * BK * G)).to(tl.int32)
...
def masking_iteration_draft_cuda_fused(
    ...
    for i_group in range(num_groups):
        ...
        while max_group_size > 1:
            ...
            n_program = tl.cdiv(BK * G * multi_branch_ratio_per_iter, BLOCK_BK)
        ...
        if multi_branch_on_layer and not multi_branch_on_once:
            tl.device_print("BUG!!!!!!!!")
            # raise Exception(f'multi_branch_on_layer: {multi_branch_on_once}, multi_branch_on_once: {multi_branch_on_once}')
        """
...
def masking_iteration_draft( 
    ...
    if (multi_branch_layer_list is not None and layer_id in multi_branch_layer_list) or \
        multi_branch_layer_all or (multi_branch_particular_layer is not None and layer_id == multi_branch_particular_layer) or \
        (multi_branch_layer_start is not None and multi_branch_layer_till is not None and layer_id >= multi_branch_layer_start and layer_id <= multi_branch_layer_till \
        and (layer_id - multi_branch_layer_start) % multi_branch_per_layer == 0):
        if multi_branch_true_iter_cnt == 1:
            if multi_branch_true_iter != 0 or (multi_branch_true_iter_str is not None and multi_branch_true_iter_str != 'first'):
                ...
                multi_branch_on_layer = False
                assert multi_branch_ratio > 1
                mask_k = mask_k * multi_branch_ratio # TODO check
    ...
    if multi_branch_true_iter == 'first':
        multi_branch_true_iter = 0
    elif multi_branch_true_iter == 'first_center':
        multi_branch_true_iter = max(math.ceil(math.log2(max_group_size) / 4), 1)
    elif multi_branch_true_iter == 'center':
        multi_branch_true_iter = max(math.ceil(math.log2(max_group_size) / 2), 1)
    elif multi_branch_true_iter == 'center_last':
        multi_branch_true_iter = max(math.ceil(math.log2(max_group_size) * (3/4)), 1)
    elif multi_branch_true_iter == 'last_last':
        multi_branch_true_iter = max(math.ceil(math.log2(max_group_size) - 2), 1)
    elif multi_branch_true_iter == 'last_last':
        multi_branch_true_iter = max(math.ceil(math.log2(max_group_size) - 1), 1) # TODO check

    if multi_branch_on_layer:
        KEY_ACCESS_LEN = mask_k * math.ceil(math.log2(max_group_size)) # log w 2 * multi_branch_ratio_per_layer might be not enough if idx_iteration != 0
        if output_key_access_log:
            key_access_log = torch.empty(
                (B, BDST, KEY_ACCESS_LEN,), dtype=torch.int32, 
                # fill_value=torch.iinfo(torch.int32).max,
    ...
    assert branch_method in ['half', 'equal_size', 'random', 'random_per_iter']
    ...
    if using_fused_iteration:
        ...
        if not scores_cached:
            ...
            BLOCK_BK = mask_k // (block_size_k // block_stride_k) * G // 8 # TODO should I change this?
    ...
    if multi_branch_on_layer or multi_branch_on_layer_first_iter:
        if multi_branch_ret_ratio_select_all:
            multi_branch_ret_ratio = multi_branch_ratio
        else:
            assert multi_branch_ret_ratio <= multi_branch_ratio
            assert multi_branch_ratio > 1
            if multi_branch_ret_ratio == 1:
                indices = indices[:mask_block_k * G]
            elif multi_branch_ret_ratio < multi_branch_ratio: # if same, no change in indices
